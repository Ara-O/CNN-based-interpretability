...
def draw_circle(img: Image.Image, size=15, pos=(10, 10), color=None, blend_alpha=0.5) -> Image.Image:
    ...
    # Draw circle on an overlay, then alpha-blend
    overlay = img.copy()
...
if __name__ == "__main__":
    ...
    # Spectral decoupling strength (logit L2 penalty)
    SD_LAMBDA     = 0.01

    CKPT_PATH     = (
        "best_alexnet_circle_small_subtle_spectral_decoupling.pt"
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, target in tqdm(train_loader, desc=f"Epoch {epoch:02d}/{EPOCHS}"):
            ...
            optimizer.zero_grad()
            logits = model(x).squeeze(1)
            # Spectral decoupling: BCE + lambda * mean(logits^2)
            loss = criterion(logits, target) + SD_LAMBDA * (logits ** 2).mean()
            loss.backward()
