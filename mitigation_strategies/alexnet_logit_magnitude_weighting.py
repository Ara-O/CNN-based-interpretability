    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, target in tqdm(train_loader, desc=f"[{run_name}] Epoch {epoch:02d}/{epochs}"):
            ...
            total_loss = criterion(logits, target) + SD_LAMBDA * (logits ** 2).mean()
            total_loss.backward()
...
if __name__ == "__main__":
    ...
    for arch_name, model_fn in ARCHITECTURES:
        for shape in SHAPES:
            run_name = f"MITIGATION_STRATEGY_MAGNITUDE_{arch_name}_{shape}_intermediate_pos{P_POS}_neg{P_NEG}"
            print(f"\n>>> Training {run_name}")
            try:
                test_m, _ = run_training_magnitude(
                    model_fn, run_name,
                    epochs=20,
            ...
            with open(os.path.join("..", "results", "summary_magnitude_weighting.json"), "w") as f:
